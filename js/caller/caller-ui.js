...
function liberarInterfaceFallback() {
    console.log('🔓 Liberando interface CALLER...');
    ...
    // 2. Mostra todos os elementos escondidos
    ...
    // 3. Garante que o vídeo local seja mostrado
    const localVideo = document.getElementById('localVideo');
    if (localVideo) {
        localVideo.style.display = 'block';
    }
    
    // 4. Mostra elementos específicos do caller
    ...
    console.log(`✅ Interface CALLER liberada - ${elementosEscondidos.length} elementos ativados`);
...
window.onload = async () => {
  ...
  setTimeout(() => {
      if (!window.interfaceLiberada) {
          console.log('🔄 Garantindo liberação da interface CALLER...');
          liberarInterfaceFallback();
      }
