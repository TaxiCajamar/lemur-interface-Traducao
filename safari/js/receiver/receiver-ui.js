...
// 🎤 SISTEMA HÍBRIDO TTS AVANÇADO (SAFARI COMPATIBLE)
let primeiraFraseTTS = true;
let navegadorTTSPreparado = false;
let ultimoIdiomaTTS = 'pt-BR';

// 🆕 🆕 🆕 SISTEMA DE ENTRADA DE TEXTO PARA SAFARI
let inputAtivo = false;
let tecladoVirtualAtivo = false;

// 🎵 CARREGAR SOM DE DIGITAÇÃO (SAFARI COMPATIBLE)
...
// 🎵 INICIAR LOOP DE DIGITAÇÃO (SAFARI COMPATIBLE)
function iniciarSomDigitacao() {
    ...
    try {
        ...
        // 🆕 SAFARI FIX: User gesture required
        const playPromise = somDigitacao.play();
        
        if (playPromise !== undefined) {
            playPromise.catch(error => {
                console.log('🔇 Safari bloqueou áudio automático:', error);
            });
        }
...
// 🎵 INICIAR ÁUDIO APÓS INTERAÇÃO DO USUÁRIO (SAFARI FIX)
function iniciarAudio() {
    if (!audioContext) {
        audioContext = new (window.AudioContext || window.webkitAudioContext)();
    }
    
    // 🆕 SAFARI: Só cria oscillator se o contexto não estiver suspenso
    ...
    console.log('🎵 Áudio desbloqueado para Safari!');
}
...
// 🆕 🆕 🆕 CONFIGURAÇÃO DO BOTÃO DE DITADO (SAFARI)
function configurarBotaoDitado() {
    const btnDitado = document.getElementById('btnDitado');
    const inputTexto = document.getElementById('inputTexto');
    
    if (!btnDitado || !inputTexto) {
        console.log('⚠️ Elementos de ditado não encontrados');
        return;
    }
    
    btnDitado.addEventListener('click', function() {
        console.log('🎤 Ativando teclado nativo Safari...');
        inputTexto.focus();
        inputTexto.select();
    });
    
    // Teclado virtual personalizado (fallback)
    inputTexto.addEventListener('focus', function() {
        tecladoVirtualAtivo = true;
        console.log('⌨️ Teclado ativado');
    });
    
    inputTexto.addEventListener('blur', function() {
        tecladoVirtualAtivo = false;
        console.log('⌨️ Teclado desativado');
    });
...
// 🆕 🆕 🆕 FUNÇÃO PARA ENVIAR MENSAGEM VIA TEXTO
function enviarMensagemTexto() {
    ...
    console.log('📤 Enviando mensagem via texto:', mensagem);
    
    // Envia via WebRTC
    if (window.rtcCore && window.rtcCore.dataChannel) {
        window.rtcCore.dataChannel.send(mensagem);
        console.log('✅ Mensagem enviada via DataChannel');
    } else {
        console.log('❌ DataChannel não disponível');
...
function inicializarSistemaTexto() {
    console.log('⌨️ Iniciando sistema de texto para Safari...');
    
    // Configura botão de ditado
    configurarBotaoDitado();
    
    // Configura botão de enviar
    const btnEnviar = document.getElementById('btnEnviar');
    if (btnEnviar) {
        btnEnviar.addEventListener('click', enviarMensagemTexto);
    }
    
    console.log('✅ Sistema de texto configurado para Safari');
}
...
async function iniciarCameraAposPermissoes() {
    try {
        ...
        // ✅✅✅ INICIA SISTEMA DE TEXTO (SAFARI) - INDEPENDENTE DA CÂMERA
        inicializarSistemaTexto();
...
document.getElementById('logo-traduz').addEventListener('click', function() {
    ...
    console.log('✅ QR Code e Link gerados/reativados!');
        ...
        // ✅ INICIA SISTEMA DE TEXTO MESMO COM ERRO
        inicializarSistemaTexto();
